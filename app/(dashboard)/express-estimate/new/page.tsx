...
interface Room {
  ...
  pedestalSink?: PedestalSinkOptions
...
interface TrimOptions {
  ...
  vinylCoveEnabled: boolean
  vinylCoveSize: string
  tileBaseEnabled: boolean
  tileBaseGrade: string
  }
...
interface DoorItem {
  ...
  mailSlot: boolean
  nonCased: boolean
  casedOpening: boolean
  casingOpeningSize: string
  casingFinish: string
  sidelites: boolean
  sidelitesQty: string
  sidelitesSize: string
  sidelitesGrade: string
  panelSize: string
  panelGrade: string
...
interface CountertopOptions {
  ...
  size: string
  detachAndReset: boolean
  /** Used for butcher-block (remove-replace | detach-reset). */
  action: string
  subdeckReplacement: boolean
}
...
const defaultRoom: Omit<Room, "id" | "name"> = {
  ...
  trim: { enabled: false, baseboardHeight: "", material: "", detail: "", finish: "", cap: false, shoe: false, shoeFinish: "", subtractCabinetry: false, vinylCoveEnabled: false, vinylCoveSize: "", tileBaseEnabled: false, tileBaseGrade: "" },
  ...
  electrical: { enabled: false, outlets110: 0, outlets220: 0, gfiOutlets: 0, lightSwitches: 0, ceilingLights: 0, ceilingFans: "", bathroomLightBar: "", bathroomLightBarQty: 0 },
...
vanity: {
  ...
  pedestalSink: { enabled: false, action: "" },
...
const defaultKitchenExtras = {
  cabinets: { enabled: false, size: "", grade: "", detachAndReset: false, toeKick: { size: "", backSplash: "", grade: "", glass: false, diagonalInstallation: false }, fullHeight: { type: "", grade: "", size: "" } },
  countertop: { enabled: false, type: "", grade: "", size: "", detachAndReset: false, action: "", subdeckReplacement: false },
...
export default function NewExpressEstimatePage() {
  ...
  /** Snapshot of local draft when opening Create New blank — used by "Use draft". */
  const pendingLocalDraftRef = useRef<string | null>(null)
  ...
  const applyDraftJson = (raw: string): boolean => {
    try {
      ...
      formRef.current.reset({
        ...
        rooms: restoredRooms.map((room) => {
          ...
          const withDefaults: Room = {
            ...r,
            notes: r.notes ?? "",
            doors: (r.doors ?? []).map((d) => ({
              ...d,
              casedOpening: d.casedOpening ?? false,
              casingOpeningSize: d.casingOpeningSize ?? "",
              casingFinish: d.casingFinish ?? "",
            })),
          }
          if (!r.appliances) return withDefaults
          return {
            ...withDefaults,
            appliances: {
  ...
  useEffect(() => {
    ...
    // Returning from preview: restore that session draft automatically.
    const previewRaw = readExpressEstimatePreviewDraft()
    if (previewRaw) {
      applyDraftJson(previewRaw)
      rehydratedRef.current = true
      return
    }
    // Create New: start blank. Offer local draft via "Use draft" if one exists.
    const localRaw = readExpressEstimateLocalDraft()
    if (localRaw) {
      pendingLocalDraftRef.current = localRaw
  ...
  const handleUseDraft = () => {
    const raw = pendingLocalDraftRef.current ?? readExpressEstimateLocalDraft()
    if (!raw) {
      setDraftAvailable(false)
      toast.error("No draft found.", { id: "ee-use-draft" })
      return
    }
    skipNextAutosaveRef.current = true
    if (!applyDraftJson(raw)) {
      toast.error("Could not restore draft.", { id: "ee-use-draft" })
      return
    }
    pendingLocalDraftRef.current = null
    setDraftAvailable(false)
    writeExpressEstimateLocalDraft(JSON.stringify(formRef.current.getValues()))
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button type="button" variant="outline" className="border-border/60" onClick={handleSave}>
            Save Draft
          </Button>
        </div>
      </div>
      ...
      {draftAvailable && (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border/60 bg-secondary/30 px-4 py-3">
          <p className="text-sm text-muted-foreground">
            You have a saved draft from a previous session. This page started blank — restore it if you want to continue that work.
          </p>
          <Button type="button" size="sm" className="gap-2 shrink-0" onClick={handleUseDraft}>
            <FileClock className="h-4 w-4" />
            Use draft
          </Button>
        </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            onCheckedChange={(checked) => {
                              setValue("exterior",{
                                ...
                                electrical: {
                                  ...exterior.electrical,
                                  meterBox: checked,
                                  ...(checked ? {} : { meterBoxSize: "", meterBoxQty: "" }),
                                },
                              })
                              handleSave()
                            }}
                          ...
                          {exterior.electrical.meterBox && (
                            <Select
                              value={exterior.electrical.meterBoxSize || "__none__"}
                              onValueChange={(__v) => {
                                const value = __v === "__none__" ? "" : __v
                                setValue("exterior",{ ...exterior, electrical: { ...exterior.electrical, meterBoxSize: value } })
                                handleSave()
                              }}
                            >
                              <SelectTrigger className="w-[240px] border-border/60 bg-secondary/50">
                                <SelectValue placeholder="Size" />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="__none__" className="italic text-muted-foreground">None</SelectItem>
                                <SelectItem value="200">Meter base - 200 AMP</SelectItem>
                                <SelectItem value="100-200">Meter base - 100- 200 AMP</SelectItem>
                                <SelectItem value="250-320">Meter base - 250- 320 AMP</SelectItem>
                                <SelectItem value="400-2000">Meter base - 400- 2000 AMP</SelectItem>
                              </SelectContent>
                            </Select>
                          )}
                        </div>
                          ...
                          {exterior.finishes.sheathing.enabled && (
                            <>
                              <div className="flex items-center gap-2">
                                ...
                                >
                                  <SelectTrigger className="h-8 w-fit min-w-[3rem] border-border/60 bg-secondary/50 text-sm">
                              ...
                              <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
                                <div className="space-y-1">
                                  ...
                                  <Select value={window.type} onValueChange={(__v) => {
                                    ...
                                    const clearAluminum = value === "double-hung" && window.material === "aluminum"
                                    newWindows[idx] = {
                                      ...window,
                                      type: value,
                                      size: "",
                                      grade: "",
                                      ...(clearAluminum ? { material: "" } : {}),
                                    }
                                    setValue("foundation",{
                                      ...foundation,
                                      subgradeAreaCoverage: {
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    handleSave()
                                      ...
                                      <SelectContent>
                                        ...
                                        {/* Aluminum - Horizontal Slider, Fixed */}
                                        ...
                                        {/* Aluminum - Casement, Single Hung */}
                                        {window.material === "aluminum" && (window.type === "casement" || window.type === "single-hung") && (
                                          <>
                                            <SelectItem value="base">Base</SelectItem>
                                            <SelectItem value="high">High Grade</SelectItem>
                                            <SelectItem value="premium">Premium Grade</SelectItem>
                    ...
                    <CollapsibleContent className="mt-2 rounded-lg border border-border/60 bg-secondary/20 p-4">
                      <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                          <div className="flex flex-wrap items-center gap-4">
                            <div className="flex items-center gap-2">
                              ...
                              />
                              <Label className="text-sm">Enable Air Handler</Label>
                            </div>
                        ...
                        <div className="space-y-3">
                          ...
                          {foundation.hvac.boiler.enabled && (
                            <div className="space-y-3 pl-6">
                              <div className="flex flex-wrap items-end gap-4">
                                <div className="space-y-1">
                                  <Label className="text-xs text-muted-foreground">Type</Label>
                        ...
                        <div className="space-y-3">
                          ...
                          {foundation.hvac.baseboardHeat.enabled && (
                            <div className="flex flex-wrap items-end gap-4 pl-6">
                              <div className="space-y-1">
                                <Label className="text-xs text-muted-foreground">Type</Label>
                                <Select value={foundation.hvac.baseboardHeat.type} onValueChange={(__v) => { const value = __v === "__none__" ? "" : __v; setValue("foundation",{ ...foundation, hvac: { ...foundation.hvac, baseboardHeat: { ...foundation.hvac.baseboardHeat, type: value, size: "", action: value === "steam-hot-water" && foundation.hvac.baseboardHeat.action === "service-call" ? "" : foundation.hvac.baseboardHeat.action } } }); handleSave() }}>
                                  <SelectTrigger className="w-32 border-border/60 bg-secondary/50">
                                    <SelectValue placeholder="Select type" />
                                  </SelectTrigger>
                                  <SelectContent>
                              ...
                              <div className="space-y-1">
                                <Label className="text-xs text-muted-foreground">{foundation.hvac.baseboardHeat.type === "steam-hot-water" ? "Size (LF)" : "Size"}</Label>
                                {foundation.hvac.baseboardHeat.type === "steam-hot-water" ? (
                                  <Input
                                    type="number"
                                    min={0}
                                    step="any"
                                    ...
                                    onChange={(e) => { setValue("foundation",{ ...foundation, hvac: { ...foundation.hvac, baseboardHeat: { ...foundation.hvac.baseboardHeat, size: e.target.value } } }); handleSave() }}
                                    className="w-[120px] border-border/60 bg-secondary/50"
                                  />
                                ) : (
                      ...
                      <div className="space-y-4">
                          ...
                          {foundation.electrical.breakerPanel.enabled && (
                            <div className="space-y-4">
                              <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end sm:gap-x-8 sm:gap-y-2">
                                <div className="space-y-2 min-w-[120px]">
                                  <Label className="text-xs text-muted-foreground">Amperage</Label>
                                  <Select value={foundation.electrical.breakerPanel.amps} onValueChange={(__v) => { const value = __v === "__none__" ? "" : __v; setValue("foundation",{ ...foundation, electrical: { ...foundation.electrical, breakerPanel: { ...foundation.electrical.breakerPanel, amps: value } } }); handleSave() }}>
                                    <SelectTrigger className="border-border/60 bg-secondary/50">
                                      <SelectValue placeholder="Select" />
                                    </SelectTrigger>
                                    <SelectContent>
                                ...
                                <div className="flex items-center gap-3 pb-0.5">
                                  <Switch
                                    ...
                                    onCheckedChange={(checked) => { setValue("foundation",{ ...foundation, electrical: { ...foundation.electrical, breakerPanel: { ...foundation.electrical.breakerPanel, arcFaults: checked } } }); handleSave() }}
                                  />
                                  <Label>w/arc fault breakers</Label>
                                </div>
                              </div>
                              <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-8 sm:gap-y-2">
                                <div className="flex items-center gap-3">
                                    ...
                                    onCheckedChange={(checked) => {
                                      setValue("foundation",{
                                        ...
                                        electrical: {
                                          ...
                                          breakerPanel: {
                                            ...foundation.electrical.breakerPanel,
                                            panelReplacement: checked,
                                            circuitReplacement: checked ? false : foundation.electrical.breakerPanel.circuitReplacement,
                                          },
                                        },
                                      })
                                      handleSave()
                                      ...
                                      setValue("foundation",{
                                        ...
                                        electrical: {
                                          ...
                                          breakerPanel: {
                                            ...foundation.electrical.breakerPanel,
                                            circuitReplacement: checked,
                                            panelReplacement: checked ? false : foundation.electrical.breakerPanel.panelReplacement,
                                          },
                                        },
                                      })
                                      handleSave()
                              ...
                              {(foundation.electrical.breakerPanel.panelReplacement ?? false) && (
                                <p className="text-xs text-muted-foreground">Panel replacement uses the amperage and arc-fault selections above.</p>
                              )}
                              {(foundation.electrical.breakerPanel.circuitReplacement ?? false) && (
                                <div className="space-y-3">
                                  <div className="flex items-end gap-2">
                                    <Button
                                      type="button"
                                  ...
                                  {foundation.electrical.breakerPanel.circuits.length > 0 && (
                                    <div className="ml-6 space-y-4 border-l border-border/40 pl-4">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          {foundation.electrical.meterBox && (
                            <Select
                              value={foundation.electrical.meterBoxSize || "__none__"}
                              onValueChange={(__v) => {
                                const value = __v === "__none__" ? "" : __v
                                setValue("foundation",{ ...foundation, electrical: { ...foundation.electrical, meterBoxSize: value } })
                                handleSave()
                              }}
                            >
                              <SelectTrigger className="w-[240px] border-border/60 bg-secondary/50">
                            ...
                            {foundation.stairs.risers && (
                              <>
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="inline-flex h-9 min-w-[10rem] items-center rounded-md border border-border/60 bg-secondary/50 px-3 text-sm text-foreground">
                                    {{
                                      "up-to-4ft": "Up to 4ft",
                                      "up-to-4ft-stain-grade": "Up to 4ft Stain Grade",
                                      "greater-4-up-to-8": "Greater than 4' up to 8'",
                                      "hardwood-up-to-4ft": "Hardwood - up to 4ft",
                                      "hardwood-up-to-4ft-high-grade": "Hardwood - up to 4ft High Grade",
                                      "hardwood-greater-4-up-to-8": "Hardwood - Greater than 4' up to 8'",
                                    }[foundation.stairs.sizeOfTreads] ?? "Select tread size"}
                                  </span>
                                </div>
                              </>
                            )}
                            ...
                            <div className="space-y-6">
                              ...
                              <div className="space-y-3 rounded-lg border border-border/40 p-4">
                                ...
                                <div className="mt-3 space-y-3 border-t border-border/40 pt-3">
                                  <Label className="text-xs font-medium text-muted-foreground">Non-wood Baseboard</Label>
                                  <div className="flex flex-wrap items-end gap-4">
                                    <div className="flex items-center gap-2 pb-1">
                                      <Switch
                                        checked={room.trim.vinylCoveEnabled}
                                        onCheckedChange={(checked) => updateRoom(room.id, { trim: { ...room.trim, vinylCoveEnabled: checked, vinylCoveSize: checked ? room.trim.vinylCoveSize : "" } })}
                                      />
                                      <Label className="text-sm">Vinyl Cove</Label>
                                    </div>
                                    {room.trim.vinylCoveEnabled && (
                                      <div className="space-y-1 min-w-[120px]">
                                        <Label className="text-xs text-muted-foreground">Size</Label>
                                        <Select value={room.trim.vinylCoveSize} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { trim: { ...room.trim, vinylCoveSize: value } }) }}>
                                          <SelectTrigger className="border-border/60 bg-secondary/50">
                                            <SelectValue placeholder="Select" />
                                          </SelectTrigger>
                                          <SelectContent>
                                            <SelectItem value="__none__" className="italic text-muted-foreground">None</SelectItem>
                                            <SelectItem value="4-wrap">4&apos; wrap</SelectItem>
                                            <SelectItem value="6-wrap">6&apos; wrap</SelectItem>
                                            <SelectItem value="8-wrap">8&apos; wrap</SelectItem>
                                          </SelectContent>
                                        </Select>
                                      </div>
                                    )}
                                    <div className="flex items-center gap-2 pb-1">
                                      <Switch
                                        checked={room.trim.tileBaseEnabled}
                                        onCheckedChange={(checked) => updateRoom(room.id, { trim: { ...room.trim, tileBaseEnabled: checked, tileBaseGrade: checked ? room.trim.tileBaseGrade : "" } })}
                                      />
                                      <Label className="text-sm">Tile Base</Label>
                                    </div>
                                    {room.trim.tileBaseEnabled && (
                                      <div className="space-y-1 min-w-[140px]">
                                        <Label className="text-xs text-muted-foreground">Grade</Label>
                                        <Select value={room.trim.tileBaseGrade} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { trim: { ...room.trim, tileBaseGrade: value } }) }}>
                                          <SelectTrigger className="border-border/60 bg-secondary/50">
                                            <SelectValue placeholder="Select" />
                                          </SelectTrigger>
                                          <SelectContent>
                                  ...
                                  <div className="space-y-3">
                                    <div className="flex flex-wrap items-end gap-4">
                                      ...
                                      {room.wallCovering.type === "paneling-unfinished" && (
                                        <>
                                          ...
                                          <div className="space-y-1">
                                            <Label className="text-xs text-muted-foreground">Finish</Label>
                                            <Select value={room.wallCovering.panelingFinish === "none" ? "" : room.wallCovering.panelingFinish} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { wallCovering: { ...room.wallCovering, panelingFinish: value } }) }}>
                                              <SelectTrigger className="w-[100px] border-border/60 bg-secondary/50">
                                                <SelectValue placeholder="Select" />
                                              </SelectTrigger>
                                              <SelectContent>
                                      ...
                                      {room.wallCovering.material === "bead-board" && (
                                        <>
                                          ...
                                          <div className="space-y-1">
                                            <Label className="text-xs text-muted-foreground">Finish</Label>
                                            <Select value={room.wallCovering.panelingFinish === "none" || room.wallCovering.panelingFinish === "stain" ? "" : room.wallCovering.panelingFinish} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { wallCovering: { ...room.wallCovering, panelingFinish: value } }) }}>
                                              <SelectTrigger className="w-[100px] border-border/60 bg-secondary/50">
                                                <SelectValue placeholder="Select" />
                                              </SelectTrigger>
                                              <SelectContent>
                                          ...
                                          <div className="space-y-1">
                                            ...
                                            <Select value={room.wallCovering.chairRailFinish === "none" ? "" : room.wallCovering.chairRailFinish} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { wallCovering: { ...room.wallCovering, chairRailFinish: value } }) }}>
                                              <SelectTrigger className="w-[100px] border-border/60 bg-secondary/50">
                                                <SelectValue placeholder="Select" />
                                              </SelectTrigger>
                                              <SelectContent>
                                              ...
                                              <SelectContent>
                                                ...
                                                {room.wallCovering.material === "plaster" && (
                                                  <>
                                                    <SelectItem value="0.5">0.5</SelectItem>
                                                    <SelectItem value="W">W</SelectItem>
                                                    <SelectItem value="pf-x-2">PF x 2</SelectItem>
                                                    <SelectItem value="pf-x-4">PF x 4</SelectItem>
                                                    <SelectItem value="pf-x-8">PF x 8</SelectItem>
                                                    <SelectItem value="pf-x-12">PF x 12</SelectItem>
                                                  </>
                                                )}
                                              </SelectContent>
                                            </Select>
                                    ...
                                    {(room.wallCovering.material === "drywall-sf" ||
                                      ...
                                      <p className="text-xs text-amber-500">
                                        Note: Each type of material selected will include the covered allotment for painting under the NFIP policy
                                      </p>
                                    )}
                                  </div>
                                )}
                                ...
                                {room.electrical.enabled && (
                                  <div className="flex flex-wrap items-end gap-x-4 gap-y-3">
                                    ...
                                    <div className="flex w-[160px] flex-col gap-1">
                                      <Label className="text-xs text-muted-foreground">Ceiling Fan</Label>
                                      <Select value={room.electrical.ceilingFans} onValueChange={(__v) => {
                                        const value = nv(__v)
                                        updateRoom(room.id, { electrical: { ...room.electrical, ceilingFans: value } })
                                      }}>
                                        <SelectTrigger className="h-9 w-full border-border/60 bg-secondary/50">
                                          <SelectValue placeholder="Select" />
                                        </SelectTrigger>
                                        <SelectContent>
                                          <SelectItem value="__none__" className="italic text-muted-foreground">None</SelectItem>
                                          <SelectItem value="fan-without-light">Fan without light</SelectItem>
                                          <SelectItem value="fan-with-light">Fan with light</SelectItem>
                                        </SelectContent>
                                      </Select>
                                    </div>
                                    <div className="flex flex-nowrap items-end gap-0">
                                      <div className="flex w-fit flex-col gap-1">
                                        ...
                                        >
                                          ...
                                          <SelectContent>
                                            <SelectItem value="__none__" className="italic text-muted-foreground">None</SelectItem>
                                            <SelectItem value="2">2 Lights</SelectItem>
                                            <SelectItem value="3">3 Lights</SelectItem>
                                            <SelectItem value="4">4 Lights</SelectItem>
                                            <SelectItem value="5">5 Lights</SelectItem>
                                            <SelectItem value="6">6 Lights</SelectItem>
                                            <SelectItem value="8">8 Lights</SelectItem>
                                          </SelectContent>
                                        </Select>
                                      </div>
                                  ...
                                  <div className="space-y-3 rounded-lg border border-border/40 p-4">
                                    <div className="flex items-center gap-3">
                                      <Switch
                                        ...
                                        onCheckedChange={(checked) =>
                                          updateRoom(room.id, {
                                            vanity: { ...room.vanity!, enabled: checked },
                                            ...(checked
                                              ? {
                                                  pedestalSink: {
                                                    ...(room.pedestalSink ?? { enabled: false, action: "" }),
                                                    enabled: false,
                                                    action: "",
                                                  },
                                                }
                                      ...
                                      />
                                      <Label className="font-medium">Vanity/Sink</Label>
                                    </div>
                                    {room.vanity.enabled && (
                                      <div className="space-y-3 pl-2">
                                        ...
                                        <div className="space-y-1.5">
                                          <div className="flex flex-wrap items-end gap-x-4 gap-y-2">
                                            ...
                                            <div className="space-y-1">
                                              <Label className="text-xs text-muted-foreground">Grade</Label>
                                              <Select value={room.vanity.grade === "custom" ? "" : room.vanity.grade} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { vanity: { ...room.vanity!, grade: value } }) }}>
                                            ...
                                            <div className="flex items-center gap-2 pb-1">
                                              <Switch
                                                checked={room.vanity.custom}
                                                onCheckedChange={(checked) => updateRoom(room.id, { vanity: { ...room.vanity!, custom: checked, grade: checked && room.vanity!.grade === "custom" ? "" : room.vanity!.grade } })}
                                              />
                                          ...
                                          <span className="text-xs text-orange-400">Note: Cabinets are put in for replacement unless D/R is selected</span>
                                        </div>
                                        {/* Countertop section */}
                                        <div className="rounded-md border border-border/30 bg-secondary/20 p-3 space-y-3">
                                          ...
                                          <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
                                            <div className="space-y-1">
                                              <Label className="text-xs text-muted-foreground">Type</Label>
                                              <Select value={room.vanity.countertop.type} onValueChange={(__v) => {
                                                const value = nv(__v)
                                                const prev = room.vanity!.countertop
                                                let nextSink = prev.sink
                                    ...
                                    <div className="space-y-3 border-t border-border/20 pt-3">
                                      <div className="flex items-center gap-3">
                                        <Switch
                                          checked={room.pedestalSink?.enabled ?? false}
                                          onCheckedChange={(checked) =>
                                            updateRoom(room.id, {
                                              pedestalSink: {
                                                ...(room.pedestalSink ?? { enabled: false, action: "" }),
                                                enabled: checked,
                                                action: checked ? (room.pedestalSink?.action ?? "") : "",
                                              },
                                              ...(checked
                                                ? { vanity: { ...room.vanity!, enabled: false } }
                                                : {}),
                                            })
                                          }
                                        />
                                        <Label className="font-medium">Pedestal Sink</Label>
                                      </div>
                                      {room.pedestalSink?.enabled && (
                                        <div className="flex flex-wrap items-end gap-x-3 gap-y-2 pl-2">
                                          <div className="space-y-1">
                                            <Label className="text-xs text-muted-foreground">Action</Label>
                                            <Select
                                              value={room.pedestalSink.action}
                                              onValueChange={(__v) => {
                                                const value = nv(__v)
                                                updateRoom(room.id, {
                                                  pedestalSink: {
                                                    ...(room.pedestalSink ?? { enabled: true, action: "" }),
                                                    action: value,
                                                  },
                                                })
                                              }}
                                  ...
                                  <div className="space-y-3 rounded-lg border border-border/40 p-4">
                                    ...
                                    {room.shower.enabled && (
                                      <div className="space-y-3">
                                        <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
                                          <div className="space-y-1">
                                            ...
                                            <Select value={room.shower.type} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { shower: { ...room.shower!, type: value } }) }}>
                                              <SelectTrigger className="min-h-9 min-w-[280px] max-w-xl w-full border-border/60 bg-secondary/50 text-left [&_[data-slot=select-value]]:line-clamp-none [&_[data-slot=select-value]]:whitespace-normal">
                                                <SelectValue placeholder="Select" />
                                              </SelectTrigger>
                                              <SelectContent>
                                                <SelectItem value="__none__" className="italic text-muted-foreground">None</SelectItem>
                                                ...
                                                <SelectItem value="fiberglass-shower-base">Fiberglass Shower Base</SelectItem>
                                                <SelectItem value="cultured-marble-shower-base">Cultured Marble Shower Base</SelectItem>
                                              </SelectContent>
                                            </Select>
                                          </div>
                                          ...
                                          {/* Tiled Shower: Action Mortar Bed (tile-shower only) */}
                                          {room.shower.type === "tile-shower" && (
                                            <>
                                                ...
                                                <div className="flex flex-wrap items-center gap-x-3 gap-y-2 min-h-9">
                                                  <div className="flex items-center gap-2">
                                                    <Switch
                                                      ...
                                                      onCheckedChange={(checked) => updateRoom(room.id, { shower: { ...room.shower!, mortarBedReplace: checked, mortarBedSize: checked ? room.shower!.mortarBedSize : "" } })}
                                                    />
                                                    <Label className="text-sm whitespace-nowrap">Replace</Label>
                                                  </div>
                                                  {room.shower.mortarBedReplace && (
                                                    <div className="flex items-center gap-2">
                                                      <Label className="text-xs text-muted-foreground whitespace-nowrap">SF to replace</Label>
                                                      <Input
                                                        type="number"
                                                        min={0}
                                                        step="0.01"
                                              ...
                                              <div className="space-y-1">
                                                <Label className="text-xs text-muted-foreground">Tile Curb</Label>
                                                <div className="flex flex-wrap items-center gap-x-3 gap-y-2 min-h-9">
                                                  <div className="flex items-center gap-2">
                                                    <Switch
                                                      checked={room.shower.tileCurb ?? false}
                                                      onCheckedChange={(checked) =>
                                                        updateRoom(room.id, {
                                                          shower: {
                                                            ...room.shower!,
                                                            tileCurb: checked,
                                                            tileCurbSize: checked ? (room.shower!.tileCurbSize ?? "") : "",
                                                          },
                                                        })
                                                      }
                                                    />
                                                    <Label className="text-sm whitespace-nowrap">Tile Curb</Label>
                                                  </div>
                                                  {room.shower.tileCurb && (
                                                    <div className="flex items-center gap-2">
                                                      <Label className="text-xs text-muted-foreground whitespace-nowrap">LF</Label>
                                                      <Input
                                                        type="number"
                                                        min={0}
                                                        step="0.01"
                                                        inputMode="decimal"
                                                        placeholder="LF"
                                                        value={room.shower.tileCurbSize ?? ""}
                                                        onChange={(e) =>
                                                          updateRoom(room.id, {
                                                            shower: { ...room.shower!, tileCurbSize: e.target.value },
                                                          })
                                                        }
                                                        className="w-[100px] border-border/60 bg-secondary/50"
                                                      />
                                          ...
                                          {/* Tiled Shower, Fiberglass Shower Base & Cultured Marble Shower Base: Walls + Shower Faucet (same options) */}
                                          {(room.shower.type === "tile-shower" || room.shower.type === "fiberglass-shower-base" || room.shower.type === "cultured-marble-shower-base") && (
                                            <>
                                              <div className="space-y-1">
                                        ...
                                        {/* Misc section - not for tub-tile-surround (has its own Misc) or fiberglass (no misc) */}
                                        {room.shower.type && room.shower.type !== "__none__" && room.shower.type !== "tub-tile-surround" && room.shower.type !== "fiberglass-tub-shower" && (
                                          <div className="space-y-3">
                                            <div>
                                ...
                                <>
                                  ...
                                  <div className="space-y-3 pl-2">
                                      ...
                                      <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
                                        ...
                                        <div className="space-y-1">
                                          ...
                                          <Input
                                            ...
                                            inputMode="decimal"
                                            placeholder="LF"
                                            value={room.cabinets.toeKick.size}
                                            onChange={(e) => updateRoom(room.id, { cabinets: { ...room.cabinets!, toeKick: { ...room.cabinets!.toeKick, size: e.target.value } } })}
                                            className="w-[100px] border-border/60 bg-secondary/50"
                                          />
                                        </div>
                                      </div>
                                    ...
                                    <div className="rounded-md border border-border/30 bg-secondary/20 p-3 space-y-2">
                                      <Label className="text-xs font-medium text-muted-foreground">Full Height Cabinetry</Label>
                                      <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
                                        <div className="space-y-1">
                                          <Label className="text-xs text-muted-foreground">Type</Label>
                                          <Select value={room.cabinets.fullHeight.type} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { cabinets: { ...room.cabinets!, fullHeight: { ...room.cabinets!.fullHeight, type: value } } }) }}>
                                            <SelectTrigger className="w-[180px] border-border/60 bg-secondary/50">
                                              <SelectValue placeholder="Select" />
                                            </SelectTrigger>
                                            <SelectContent>
                                              <SelectItem value="__none__" className="italic text-muted-foreground">None</SelectItem>
                                              <SelectItem value="cab-full-height">Cab - full height</SelectItem>
                                              <SelectItem value="custom-cab-full-height">Custom cab full height</SelectItem>
                                            </SelectContent>
                                          </Select>
                                        </div>
                                        <div className="space-y-1">
                                          <Label className="text-xs text-muted-foreground">Grade</Label>
                                          <Select value={room.cabinets.fullHeight.grade} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { cabinets: { ...room.cabinets!, fullHeight: { ...room.cabinets!.fullHeight, grade: value } } }) }}>
                                            <SelectTrigger className="w-[110px] border-border/60 bg-secondary/50">
                                              <SelectValue placeholder="Select" />
                                            </SelectTrigger>
                                            <SelectContent>
                                        ...
                                        <div className="space-y-1">
                                          ...
                                          <Input
                                            ...
                                            inputMode="decimal"
                                            placeholder="LF"
                                            value={room.cabinets.fullHeight.size}
                                            onChange={(e) => updateRoom(room.id, { cabinets: { ...room.cabinets!, fullHeight: { ...room.cabinets!.fullHeight, size: e.target.value } } })}
                                            className="w-[100px] border-border/60 bg-secondary/50"
                                          />
                                        </div>
                                      </div>
                                    ...
                                    <div className="rounded-md border border-border/30 bg-secondary/20 p-3 space-y-2">
                                      ...
                                      <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
                                        <div className="space-y-1">
                                          <Label className="text-xs text-muted-foreground">Type</Label>
                                          <Select value={room.countertop.type} onValueChange={(__v) => {
                                            const value = nv(__v)
                                            const prev = room.countertop!
                                            const nextGrade =
                                              value === "cultured-marble" && (prev.grade === "premium" || prev.grade === "standard")
                                                ? ""
                                                : value === "tile" && prev.grade === ""
                                                ? prev.grade
                                                : prev.grade
                                            let nextAction = prev.action
                                            let nextDetach = prev.detachAndReset
                                            if (value === "butcher-block") {
                                              if (prev.action !== "remove-replace" && prev.action !== "detach-reset") {
                                                nextAction = prev.detachAndReset ? "detach-reset" : "remove-replace"
                                              }
                                              nextDetach = nextAction === "detach-reset"
                                            } else if (prev.type === "butcher-block") {
                                              nextAction = ""
                                            }
                                            if (value === "tile") {
                                              nextDetach = false
                                            ...
                                            updateRoom(room.id, {
                                              countertop: {
                                                ...
                                                subdeckReplacement: value === "tile" ? true : prev.subdeckReplacement,
                                              },
                                            })
                                          }}>
                                        ...
                                        <div className="space-y-1">
                                          ...
                                          <Select value={room.countertop.grade} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { countertop: { ...room.countertop!, grade: value } }) }}>
                                            ...
                                            <SelectContent>
                                              <SelectItem value="__none__" className="italic text-muted-foreground">None</SelectItem>
                                              <SelectItem value="base">Base</SelectItem>
                                              {room.countertop.type === "cultured-marble" ? (
                                                <SelectItem value="high">High</SelectItem>
                                              ) : (
                                                <>
                                                  <SelectItem value="standard">Standard</SelectItem>
                                                  {room.countertop.type === "tile" && (
                                                    <SelectItem value="high">High</SelectItem>
                                                  )}
                                                  <SelectItem value="premium">Premium</SelectItem>
                                                </>
                                        ...
                                        <div className="space-y-1">
                                          ...
                                          <Input
                                            ...
                                            step="any"
                                            inputMode="decimal"
                                            placeholder={room.countertop.type === "cultured-marble" || room.countertop.type === "laminate" ? "LF" : "SF"}
                                        ...
                                        {room.countertop.type === "butcher-block" ? (
                                          <div className="space-y-1">
                                            <Label className="text-xs text-muted-foreground">Action</Label>
                                            <Select
                                              value={room.countertop.action || "__none__"}
                                              onValueChange={(__v) => {
                                                const value = nv(__v)
                                                updateRoom(room.id, {
                                                  countertop: {
                                                    ...room.countertop!,
                                                    action: value,
                                                    detachAndReset: value === "detach-reset",
                                                  },
                                                })
                                              }}
                                            >
                                        ...
                                        <div className="flex items-center gap-2 pb-1">
                                          <Switch
                                            checked={room.countertop.subdeckReplacement}
                                            onCheckedChange={(checked) => updateRoom(room.id, { countertop: { ...room.countertop!, subdeckReplacement: checked } })}
                                          />
                                          <Label className="text-xs whitespace-nowrap">Subdeck replacement</Label>
                                        </div>
                                      </div>
                                    </div>
                                    ...
                                    <div className="rounded-md border border-border/30 bg-secondary/20 p-3 space-y-2">
                                      ...
                                      <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
                                        <div className="space-y-1">
                                          <Label className="text-xs text-muted-foreground">Type</Label>
                                          <Select value={room.cabinets.toeKick.backSplash === "unattached" ? "" : room.cabinets.toeKick.backSplash} onValueChange={(__v) => {
                                            const value = nv(__v)
                                            updateRoom(room.id, {
                                              cabinets: {
                                                ...room.cabinets!,
                                                toeKick: {
                                                  ...room.cabinets!.toeKick,
                                                  backSplash: value,
                                                  ...(value === "solid-surface"
                                                    ? { grade: "", glass: false, diagonalInstallation: false }
                                                    : {}),
                                                },
                                              },
                                            })
                                        ...
                                        {room.cabinets.toeKick.backSplash !== "solid-surface" && (
                                        <>
                                        <div className="space-y-1">
                                          <Label className="text-xs text-muted-foreground">Grade</Label>
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        {room.plumbing.waterSupplyLine.enabled && (
                                          <Input
                                            ...
                                            inputMode="numeric"
                                            placeholder="QTY"
                                            value={room.plumbing.waterSupplyLine.qty}
                                            onChange={(e) => updateRoom(room.id, { plumbing: { ...room.plumbing!, waterSupplyLine: { ...room.plumbing!.waterSupplyLine, qty: e.target.value } } })}
                                            className="border-border/60 bg-secondary/50 w-[80px]"
                                          />
                                        )}
                                      </div>
                                    </div>
                                      ...
                                      {room.plumbing.reverseOsmosis.enabled && (
                                        <>
                                          ...
                                          <Input
                                            ...
                                            className="border-border/60 bg-secondary/50 w-[200px]"
                                          />
                                        </>
                                      )}
                                    </div>
                                    ...
                                    {room.appliances.enabled && (
                                      <div className="space-y-3">
                                          ...
                                          {room.appliances.waterHeater.enabled && (
                                            <div className="flex flex-wrap items-center gap-3">
                                              <Select
                                                ...
                                                onValueChange={(__v) => {
                                                  const value = nv(__v)
                                                  const clearElectricOnlySize = value === "gas" && room.appliances!.waterHeater.size === "80"
                                                  updateRoom(room.id, {
                                                    appliances: {
                                                      ...room.appliances!,
                                                      waterHeater: {
                                                        ...room.appliances!.waterHeater,
                                                        type: value,
                                                        ...(clearElectricOnlySize ? { size: "" } : {}),
                                                      },
                                                    },
                                                  })
                                                }}
                                              >
                                                ...
                                                <SelectContent>
                                                  <SelectItem value="__none__" className="italic text-muted-foreground">None</SelectItem>
                                                  {room.appliances.waterHeater.size !== "80" && (
                                                    <SelectItem value="gas">Gas</SelectItem>
                                                  )}
                                                  <SelectItem value="electric">Electric</SelectItem>
                                                </SelectContent>
                                              ...
                                              >
                                                ...
                                                <SelectContent>
                                                  ...
                                                  {room.appliances.waterHeater.type === "electric" && (
                                            ...
                                            <div className="space-y-3">
                                              <div className="flex flex-wrap items-end gap-3">
                                                ...
                                                <Input
                                                  ...
                                                  className="border-border/60 bg-secondary/50 w-[180px]"
                                                />
                                              </div>
                                              <div className="flex flex-wrap items-center gap-4">
                                                <div className="flex items-center gap-2">
                                        ...
                                        <div className="space-y-3 rounded-lg bg-secondary/30 p-3">
                                          <div className="flex items-center gap-3">
                                            <Switch
                                              checked={room.appliances.baseboardHeat.enabled}
                                              onCheckedChange={(checked) => updateRoom(room.id, { appliances: { ...room.appliances!, baseboardHeat: { ...room.appliances!.baseboardHeat, enabled: checked } } })}
                                            />
                                            <Label className="text-sm font-medium">Baseboard Heat</Label>
                                          </div>
                                          {room.appliances.baseboardHeat.enabled && (
                                            <div className="flex flex-wrap items-end gap-3">
                                              <div className="space-y-1">
                                                <Label className="text-xs text-muted-foreground">Type</Label>
                                                <Select value={room.appliances.baseboardHeat.type} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { appliances: { ...room.appliances!, baseboardHeat: { ...room.appliances!.baseboardHeat, type: value, size: "" } } }) }}>
                                              ...
                                              <div className="space-y-1">
                                                <Label className="text-xs text-muted-foreground">Action</Label>
                                                <Select value={room.appliances.baseboardHeat.action} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { appliances: { ...room.appliances!, baseboardHeat: { ...room.appliances!.baseboardHeat, action: value } } }) }}>
                                                  <SelectTrigger className="border-border/60 bg-secondary/50 text-sm w-[120px]">
                                                    <SelectValue placeholder="Select" />
                                                  </SelectTrigger>
                                                  <SelectContent>
                              ...
                              {/* Appliance / Misc Equipment - for all room types except kitchen (which has its own appliances section) */}
                              {room.type !== "kitchen" && room.appliances && (
                                <div className="space-y-3 rounded-lg border border-border/40 p-4">
                                  <div className="flex items-center gap-3">
                                    <Switch
                                      checked={room.appliances.enabled}
                                          ...
                                          <div className="flex flex-wrap items-center gap-3">
                                            <Select
                                              ...
                                              onValueChange={(__v) => {
                                                const value = nv(__v)
                                                const clearElectricOnlySize = value === "gas" && room.appliances!.waterHeater.size === "80"
                                                const clearGas75Rating = value === "gas" && room.appliances!.waterHeater.size === "75"
                                                updateRoom(room.id, {
                                                  appliances: {
                                                    ...room.appliances!,
                                                    waterHeater: {
                                            ...
                                            <Select
                                              ...
                                              onValueChange={(__v) => {
                                                ...
                                                const clearGas75Rating =
                                                  room.appliances!.waterHeater.type === "gas" && value === "75"
                                                updateRoom(room.id, {
                                                  appliances: {
                                                    ...room.appliances!,
                                                    waterHeater: {
                                            ...
                                            {!(
                                              room.appliances.waterHeater.type === "gas" &&
                                              room.appliances.waterHeater.size === "75"
                                            ) && (
                                            <Select value={room.appliances.waterHeater.rating} onValueChange={(__v) => { const value = nv(__v); updateRoom(room.id, { appliances: { ...room.appliances!, waterHeater: { ...room.appliances!.waterHeater, rating: value } } }) }}>
                                              <SelectTrigger className="border-border/60 bg-secondary/50 text-sm w-[100px]">
                                                <SelectValue placeholder="Rating" />
                                        ...
                                        <div className="flex flex-wrap items-center gap-3">
                                          <Select value={door.type} onValueChange={(__v) => {
                                            ...
                                            <SelectContent>
                                              ...
                                              <SelectItem value="slab-only-refit">Slab only - refit</SelectItem>
                                            </SelectContent>
                                          </Select>
                                          {door.type !== "slab-only-refit" && (
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    {room.doors.map((door, idx) => door.category === "exterior" && (
                                      <div key={door.id} className="space-y-3 rounded-lg bg-secondary/30 p-3">
                                        <div className="flex flex-wrap items-center gap-3">
                                          <Select value={door.type} onValueChange={(__v) => {
                                            ...
                                            newDoors[idx] = {
                                              ...
                                              grade: "",
                                              size: "",
                                              ...(value !== "overhead" ? { cleanSensor: false, replaceSensor: false } : {}),
                                            }
                                            updateRoom(room.id, { doors: newDoors })
                                          }}>
                                            <SelectTrigger className="border-border/60 bg-secondary/50 text-sm w-[140px]">
                                        ...
                                        {door.type === "overhead" && (
                                          <div className="flex flex-wrap items-center gap-6 border-t border-border/20 pt-2">
                                            <div className="flex items-center gap-2">
                                              <Switch
                                                checked={door.cleanSensor ?? false}
                                                onCheckedChange={(checked) => {
                                                  const newDoors = [...room.doors]
                                                  newDoors[idx] = { ...door, cleanSensor: checked, replaceSensor: checked ? false : (door.replaceSensor ?? false) }
                                                  updateRoom(room.id, { doors: newDoors })
                                                }}
                                              />
                                              <Label className="text-sm whitespace-nowrap">Clean Sensor</Label>
                                            </div>
                                            <div className="flex items-center gap-2">
                                              <Switch
                                                checked={door.replaceSensor ?? false}
                                                onCheckedChange={(checked) => {
                                                  const newDoors = [...room.doors]
                                                  newDoors[idx] = { ...door, replaceSensor: checked, cleanSensor: checked ? false : (door.cleanSensor ?? false) }
                                                  updateRoom(room.id, { doors: newDoors })
                                                }}
                                              />
                                              <Label className="text-sm whitespace-nowrap">Replace Sensor</Label>
                                            </div>
                                          </div>
                                        )}
                                            ...
                                            <Switch
                                              ...
                                              onCheckedChange={(checked) => {
                                                ...
                                                newDoors[idx] = {
                                                  ...door,
                                                  sidelites: checked,
                                                  sidelitesQty: checked ? door.sidelitesQty : "",
                                                  sidelitesSize: checked ? door.sidelitesSize : "",
                                                  sidelitesGrade: checked ? door.sidelitesGrade : "",
                                                  panelSize: "",
                                                  panelGrade: "",
                                                }
                                                updateRoom(room.id, { doors: newDoors })
                                            ...
                                            <>
                                              {door.type !== "overhead" && (
                                                <>
                                                  <div className="space-y-1">
                                                    <Label className="text-xs text-muted-foreground">Size</Label>
                                                    <Select value={door.sidelitesSize} onValueChange={(__v) => {
                                                      const value = __v === "__none__" ? "" : __v;
                                                      const newDoors = [...room.doors]
                                                      newDoors[idx] = { ...door, sidelitesSize: value }
                                                      updateRoom(room.id, { doors: newDoors })
                                                    }}>
                                                      <SelectTrigger className="border-border/60 bg-secondary/50 text-sm w-[100px]">
                                                        <SelectValue placeholder="Select" />
                                                  ...
                                                  <div className="space-y-1">
                                                    <Label className="text-xs text-muted-foreground">Grade</Label>
                                                    <Select value={door.sidelitesGrade} onValueChange={(__v) => {
                                                      const value = __v === "__none__" ? "" : __v;
                                                      const newDoors = [...room.doors]
                                                      newDoors[idx] = { ...door, sidelitesGrade: value }
                                                      updateRoom(room.id, { doors: newDoors })
                                                    }}>
                                                      <SelectTrigger className="border-border/60 bg-secondary/50 text-sm w-[120px]">
                                                        <SelectValue placeholder="Select" />
